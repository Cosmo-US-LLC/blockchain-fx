...
export const groupTokens = (tokens) => {
  ...
  return Object.entries(grouped).map(([key, value]) => {
    const getSortValue = ([token, i]) =>
      token.symbol.toUpperCase() === value.defaultTokenSymbol?.toUpperCase()
        ? -1
        : i;
    return {
...
export const getTopTokenList = (tokens) => {
  let list = [
    ...
    tokens.find(
      (token) =>
        ...
        token.chain.toUpperCase() === "BEP-20"
    ),
    tokens.find(
      (token) =>
  ...
  for (let i = list.length - 1; i < 3; i++) {
    const newToken = tokens.find((token) =>
      list.every((token2) => token2.id !== token.id)
    );
    if (!newToken) break;
    list.push(newToken);
  }
