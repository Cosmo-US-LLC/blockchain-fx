...
function PaymentCardTwoMobile() {
  return (
    <div className=" bg-[#0F0F0F] border border-[#3F3F3F] rounded-xl px-2 py-3 flex items-center gap-2 w-full  ">
      ...
      <div className="flex flex-col gap-2">
        <button className="bg-[#1C1C1C] w-[180px]  rounded-lg px-4 py-1 flex items-center gap-3 text-white text-[6px] font-[600] justify-center hover:bg-[#2A2A2A] transition">
          <img src={visaIcon} alt="Visa" className="w-5 h-5" />
          <img src={mastercardIcon} alt="Mastercard" className="w-5 h-5" />
          Buy with Card
        </button>
        <div className="flex flex-wrap w-[180px]  justify-center gap-3">
        {currencyOptions.map((item, idx) => (
          <div
            key={idx}
            className="bg-[#1C1C1C] rounded-lg w-[48px] flex flex-col items-center gap-1 px-2 py-2 text-white text-[5px] font-[500] hover:bg-[#2A2A2A] transition"
          >
            {item.icon ? (
            ...
            <span className="font-semibold text-[6px]">{item.label}</span>
            {item.sub && (
              <span className="text-[6px] text-[#9F9F9F] font-light">
                {item.sub}
              </span>
            )}
          </div>
