...
function PremiumSec() {
  ...
  // Create cards from translation data
  const cards = Array.from({ length: 7 }).map((_, index) => ({
    id: index + 1,
    img: cardImages[index],
    title: t(`premium_section.cards.${index}.title`),
    price: t(`premium_section.cards.${index}.price`),
    reviews: Array(7)
      .fill(false)
      .map((_, i) => i < index + 1),
    points: t(`premium_section.cards.${index}.points`, { returnObjects: true }),
    buttonLabel: t(`premium_section.cards.${index}.buttonLabel`),
  }))

  // Create FAQ bundles from translation data
  const Bundles = Array.from({ length: 4 }).map((_, index) => ({
    id: index + 1,
    question: t(`premium_section.faq.questions.${index}.question`),
    answer: t(`premium_section.faq.questions.${index}.answer`),
    tooltip: index === 3, // Only the 4th question has a tooltip
  }))
              ...
              {cards.map((card, index) => (
                <SwiperSlide key={card.id}>
                  <div
                    ...
                    className={`p-[15px] rounded-[8px] flex w-[100%] mx-auto flex-col justify-between ${
                      activeIndex === index ? "border-black" : "border-[rgba(230, 175, 3, 0.12)]"
                    } border !min-h-[599px] max-h-[599px] h-[100%]`}
                  >
                    <div>
                      <img
                        ...
                        <div>
                          <h3 className="text-[#ffff] flex flex-col justify-end font-[600] ">
                            <span
                              className="text-[40px] leading-[100%]"
                              style={{
                                background: "linear-gradient(90deg, #FFF 0%, #030F16 100%)",
                                backgroundClip: "text",
                                WebkitBackgroundClip: "text",
                                WebkitTextFillColor: "transparent",
                              }}
                    ...
                    <div className="flex justify-start">
                      <button
                        ...
                        style={{
                          background:
                            hoveredIndex === index ? "transparent" : "linear-gradient(90deg, #E5AE00 0%, #FFD551 100%)",
                        }}
                        className="text-black px-[12px] hover:text-[#E5AE00] hover:!bg-transparent text-[15.27px] font-[800] border border-[#E5AE00] hover:border-[#E5AE00] rounded-[8px] w-[auto] h-[49px]"
                      >
                        {card.buttonLabel}
        ...
        >
          ...
          <div className="flex justify-between pt-[8px] pl-2">
            {cards.map((card, index) => (
              <div
                key={index}
                className={`w-[100%] max-w-[${index === 0 || index === 2 || index === 5 ? "70px" : index === 1 || index === 3 ? "120px" : index === 4 ? "100px" : "98px"}`}
              >
                <h4 className="text-[16px] text-[#fff] font-[600] leading-[115%] text-center">{card.title}</h4>
                ...
                >
                  ...
                  <span className="text-xl font-bold">
                    {openBundles === item.id ? (
                      ...
                      <img className="rotate-[180deg]" src={faq1 || "/placeholder.svg"} alt="" />
                    )}
                  </span>
                </div>
                {openBundles === item.id && (
                        ...
                        >
                          ...
                          {showTooltip && (
                            <div className="absolute top-[13px] z-[99] bg-[#808080] w-[260px] text-start leading-[110%] text-[#fff] text-[10px] font-[400] rounded-md p-[10px] shadow-lg">
                              ...
                              {t("premium_section.faq.tooltip_text.line1")} <br />{" "}
                              <span className="absolute top-[28px] left-[4px]">.</span>{" "}
                              {t("premium_section.faq.tooltip_text.line2")} <br />{" "}
                              <span className="absolute top-[40px] left-[4px]">.</span>{" "}
                              {t("premium_section.faq.tooltip_text.line3")}
                            </div>
                          )}
                        </div>
                      </div>
